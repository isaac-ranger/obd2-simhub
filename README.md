...
* **`common`** holds values shared by more than one tool. A value that
  ...
  the rename. (A config predating the GPS leg needs one edit per bare key —
  `port` → `obd_port`, `baud` → `obd_baud` — and each error message tells
  you exactly which.) A section
...
captures on the reference car, engine warm, one per mode (normal / individual
/ sport / sport+, n = 88 / 76 / 95 / 99 samples, ~20 s each, every sample at
0 km/h in gear 0): each pair of modes that shares an idle speed is
indistinguishable — means 0.02 and 0.003 points apart, and a capture this
short cannot see anything under ~0.1 — and all four sat on the same **11.8 %**
step at the median that morning. Sport and sport+ idle about 120 rpm higher
(~805 vs ~685) and average 0.1 ± 0.03 points more: a third of the PID's own
0.4-point step, and less than the same idle has drifted between sessions on
this car as the engine warmed. Mode sets idle rpm, idle rpm nudges the plate
— a little — and the learner's selectors already run on speed, load and rpm
and never on mode, so learn in whichever mode you drive; at idle, at least,
